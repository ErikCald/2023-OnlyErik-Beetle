 ...
 * call {@link DifferentialDrivePoseEstimatorExposed#addVisionMeasurement} and only call {@link
 * DifferentialDrivePoseEstimatorExposed#update} then this will behave exactly the same as
 * DifferentialDriveOdometry.
 *
 * <p>{@link DifferentialDrivePoseEstimatorExposed#update} should be called every robot loop.
 *
 * <p>{@link DifferentialDrivePoseEstimatorExposed#addVisionMeasurement} can be called as infrequently as
 * you want; if you never call it then this class will behave exactly like regular encoder odometry.
 */
public class DifferentialDrivePoseEstimatorExposed {
  ...
  */
  ...
  public Optional<Pose2d> getPoseAtTimestamp(double timestampSeconds) {
    Optional<InterpolationRecord> interpRecord = m_poseBuffer.getSample(timestampSeconds);
    if (interpRecord.isEmpty()) {
      return Optional.empty();
    } else {
      return Optional.of(interpRecord.get().poseMeters);
    }
  }

  /**
   * Constructs a DifferentialDrivePoseEstimatorExposed with default standard deviations for the model and
   * vision measurements.
   *
   ...
   */
  public DifferentialDrivePoseEstimatorExposed(
  ...
  /**
   * Constructs a DifferentialDrivePoseEstimatorExposed.
   *
   * @param kinematics A correctly-configured kinematics object for your drivetrain.
   ...
   * DifferentialDrivePoseEstimatorExposed#update} every loop.
   *
   ...
   *     DifferentialDrivePoseEstimatorExposed#updateWithTime(double,Rotation2d,double,double)} then you
   ...
   * DifferentialDrivePoseEstimatorExposed#setVisionMeasurementStdDevs(Matrix)} or this method.
   *
   * @param visionRobotPoseMeters The pose of the robot as measured by the vision camera.
   * @param timestampSeconds The timestamp of the vision measurement in seconds. Note that if you
   *     don't use your own time source by calling {@link
